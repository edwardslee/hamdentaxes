...
df <- read_rds("02-17-26_allstreets.rds")
...
ui <- fluidPage(
  ...
  tags$div(
    id = "above-tabs-text",
    h2("Hamden Tax Calculator 2026"),

    # ---- one tab text
    p("This app helps you estimate your property taxes for 2025-2026.
      The calculator uses the Mayor's recommended mill rate of 54.43 and calculates property taxes for the upcoming year based on the 2nd year of the 4 year phase-in after the Revaluation that began last year."),
    p(strong("This calculator is not official and was developed by fellow Hamden homeowners.")),
    br()
  ),
  # ====== Tabs ======
  tabsetPanel(
    id = "new_tabs",
    tabPanel(
      title = "2025 Tax Calculator (Final)",
      value = "prop2025_tab",
      # Add your custom UI elements here for the blank tab
      fluidRow(
        column(12, class = "top-section",
               h3("2026–2027 Property Tax Calculator"),
               p("Estimate your property taxes for the upcoming year with the mill rate of 54.43 and the 2nd year of the phase in."),
               p("To use the calculator:"), 
               tags$li("On the left, search for an address and then choose one from the dropdown menu"),
               tags$li("You can change the assessed value of your property if the listed assessment is not correct or has not been updated. (CURRENTLY NOT WORKING)")
        )
      ),
      ...
      sidebarLayout(
        ...
        sidebarPanel(
          ...
          textInput("address_search_tab_new", "Search for your address", ""),
          selectInput("address_tab_new", "Choose your address", choices = NULL),
          # Include option to change the assessed value (for folks who disputed their reval)
          numericInput("custom_assessment_tab_new", 
                       "Enter your assessed value (optional):", 
                       value = NA, min = 0, step = 1000),
          helpText("Leave blank to use the town’s official value from the online database.")
        ),
        ...
        mainPanel(
          ...
          h4("Estimated Taxes"),
          htmlOutput("string_tab_new"),
          div(class = "output-padding", htmlOutput("string_tab_new")),
    ...
    #            tags$li("Council's Budget: The Council's budget is proposed to have a total of $304,790,424.29 with a mill rate of 52.16 (with the 4-year phase in). The actual total property tax revenue ask is ???."),
...
server <- function(input, output, session) {
  ...
  filtered_addresses_tab_new <- reactive({
    # Get the search term
    search_term <- input$address_search_tab_new
  ...
  observe({
    choices_tab_new <- filtered_addresses_tab_new()
    updateSelectInput(session, "address_tab_new", choices = choices_tab_new)
  })
  
  home_chooser_tab_new <- reactive({df |>
      filter(address == input$address_tab_new)
  })
  
  # Calculate property tax based on chosen mill rate
  calculated_tax_tab_new <- reactive({
    req(input$address_tab_new)
    home_data <- home_chooser_tab_new()
    ...
    assessment_new <- if (!is.na(input$custom_assessment_tab_new) && input$custom_assessment_tab_new > 0) {
      input$custom_assessment_tab_new
    } else {
      home_data$assessment_new
    }
    
    # year 1 of phase-in
    assessment_year1 <- home_data$assessment_year1
    tax_year1 <- 51.88 / 1000 * assessment_year1 
    
    # year 2 of phase-in
    assessment_year2 <- home_data$assessment_year2
    tax_year2 <- 54.43 / 1000 * assessment_year2
    
    

    list(
      assessment_year1 = assessment_year1,
      tax_year1 = tax_year1,
      assessment_year2 = assessment_year2,
      tax_year2 = tax_year2,
      diff = tax_year2 - tax_year1,
      perc_inc = (tax_year2 - tax_year1) / tax_year1 * 100,
      monthly_inc = (tax_year2 - tax_year1) / 12
    )
  })
  
  output$string_tab_new <- renderText({
    req(input$address_tab_new)  # Only proceed if an address is selected
    
    df_home <- home_chooser_tab_new()
    
    # Check if we have valid data
    if (nrow(df_home) == 0) return("Please select a valid address")
    
    tax_data <- calculated_tax_tab_new()
    
    
    tax_year1 <- tax_data$tax_year1 |>
      round() |>
      formatC(format="d", big.mark=",")
    tax_year2 <- tax_data$tax_year2 |>
      round() |>
      formatC(format="d", big.mark=",")
    assessment_year1 <- tax_data$assessment_year1 |> formatC(format="d", big.mark=",")
    assessment_year2 <- tax_data$assessment_year2 |> formatC(format="d", big.mark=",")
    diff <- tax_data$diff |> round()
    perc_inc <- tax_data$perc_inc |> 
      round() |> 
      formatC(digits = 2, format = "f")
    monthly_inc <- tax_data$monthly_inc |>
      round() |> 
      formatC(digits = 2, format = "f")
    

    if (diff > 0) {
      string <- str_c("<b>Estimated Taxes for 2026-2027:</b> ",
                      "Your property taxes for 2026-2027 are estimated to be <b>$",
                      tax_year2,
                      "</b>. This is an increase of $",
                      diff,
                      " (",
                      perc_inc, 
                      "%) compared to last year. ",
                      "Your monthly payment would increase by $", monthly_inc, ".<br/><br/>",
                      
                      "The net assessment on your home for the 2nd year of the phase-in is $",
                      assessment_year2, ".<br/><br/>",
                      
                      "<b>Last year's information:</b> ",
                      "The net assessment on your home for last year was $", 
                      assessment_year1, 
                      ", and property taxes on your property last year was $",
                      tax_year1,
                      "."
                      )
    } else {
      diff <- diff(monthly_payment_phasein, "^-+", "")
      perc_inc   <- str_replace(perc_inc, "^-+", "")
      
      string <- str_c("Your property taxes for 2026-2027 are estimated to be <b>$",
                      tax_year2,
                      "</b>. This is an decrease of $",
                      diff,
                      " and a ",
                      perc_inc, 
                      "% decrease compared to last year.:<br/><br/>",
                      
                      "The net assessment for your home for the 2nd year of the phase-in is $",
                      assessment_year2, ".",
                      "The net assessment for your home for 2025-2026 was $", 
                      assessment_year1, 
                      ", and property taxes in 2025-2026 was $",
                      tax_year1,
                      "."
      )
    }
  ...
  #   assessment_old <- (appraisal_old * 0.7) |> round() |> formatC(format="d", big.mark=",")
  #   assessment_new <- (appraisal_new * 0.7) |> round() |> formatC(format="d", big.mark=",")
  ...
  #                     prop_tax_old,
  #                     ", and your property's appraisal value was $",
  #                     appraisal_old, " and assessment was $", assessment_old, ". Your property's new appraisal value is $", appraisal_new, " and assessment was $",
  #                     assessment_new, ". If the assessment number is not correct for your property, edit the assessment value on the left sidebar. <br/><br/>", 
  #                     
