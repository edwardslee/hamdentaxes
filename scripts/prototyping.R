...
df <- df_raw |>
  mutate(
    assessment_new = appraisal * 0.7,
    assessment_old = hist_total_value * 0.7,
    property_tax_new =  
      (assessment_old + ((assessment_new - assessment_old)*(phase_in_year/phase_in_term))) * mill_rate_new / 1000 |> round(),
...
string1 <- str_c("With the proposed mill rate of 52.1, in the ",
                 phase_in_year_natural_lang,
                 " year of the ", 
                 phase_in_term,
                 "-year reevaluation phase-in, your property taxes will be $",
                 prop_tax_new,
